...
Repeated builds reuse unchanged Rust artifacts. The renderer still runs type checking and Vite, but preserves identical output files and timestamps; temporary Nikon bundle paths are applied only after compilation. Release optimization remains unchanged. A first build or a change to Rust, embedded frontend assets, dependencies, or build configuration can still require a full compile. [Build measurements](docs/performance/README.md#packaging-builds) include the measured improvement and its limits.
