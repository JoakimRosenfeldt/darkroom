...
pub use metadata::{MetadataPlan, run_metadata_analysis};
mod trash_duplicates;
#[allow(unused_imports)]
pub use trash_duplicates::{ExactDuplicateTrashPlan, run_exact_duplicate_trash};
...
fn upgrade_v3(db: &Connection) -> Result<(), String> {
    for table in [
        ...
        .is_none()
        {
            return Err(format!("Catalog v3 table {table} is missing."));
        }
    }
    db.execute_batch(SCHEMA).map_err(|e| e.to_string())?;
    for column in ["display_name", "parent_entry_id", "tombstoned_at"] {
        if !rows(db, "PRAGMA table_info(edit_entries)", vec![])?
            .iter()
            .any(|v| v["name"] == column)
        {
            db.execute_batch(&format!(
                "ALTER TABLE edit_entries ADD COLUMN {column} {};",
                if column == "tombstoned_at" {
                    "REAL"
                } else {
                    "TEXT"
                }
...
pub(crate) fn field<'a>(value: &'a Value, key: &str) -> Result<&'a Value, String> {
    value
        .get(key)
        .ok_or_else(|| format!("Catalog {key} is missing."))
}
...
pub(crate) fn first(args: &Value) -> Result<&Value, String> {
    args.as_array()
        .and_then(|a| a.first())
        .ok_or_else(|| "Catalog request is missing.".into())
}
...
pub(crate) fn rows(
    ...
    let selected = statement
        .query_map(params_from_iter(values), row_object)
        .map_err(|e| e.to_string())?;
    selected
...
pub(crate) fn values(fields: &[&Value]) -> Vec<SqlValue> {
    fields.iter().map(|v| sql_value(v)).collect()
}
...
impl CatalogService {
    pub(crate) fn for_worker(db: Connection) -> Self {
        Self {
            ...
            registry: json!({"version":1,"catalogs":[]}),
            active: None,
            db: Some(db),
            operations: Arc::new(Mutex::new(HashMap::new())),
            scan_handles: Vec::new(),
            fingerprint_jobs: Arc::new(Mutex::new(HashMap::new())),
            relink_drafts: HashMap::new(),
            watcher_stops: Arc::new(Mutex::new(Vec::new())),
            watcher_handles: Vec::new(),
            import_drafts: HashMap::new(),
            import_jobs: HashMap::new(),
            auto_stop: Arc::new(AtomicBool::new(false)),
            auto_guard: Arc::new(Mutex::new(())),
            auto_cancellations: Arc::new(Mutex::new(HashMap::new())),
            auto_handle: None,
            emit: None,
    ...
    pub fn new(user_data: PathBuf) -> Result<Self, String> {
        ...
        let registry = match fs::read_to_string(registry_path) {
            ...
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
                json!({"version": 1, "catalogs": []})
            }
            Err(error) => return Err(error.to_string()),
        };
        if registry.get("version") != Some(&json!(1))
            || !registry.get("catalogs").is_some_and(Value::is_array)
        {
    ...
    pub fn dispatch(&mut self, command: &str, args: Value) -> Result<Value, String> {
        match command {
            ...
            "darkroom:catalog-open" | "darkroom:catalog-switch" => {
                self.open(string(first(&args)?, "catalogId")?)
            }
            "darkroom:catalog-close" => {
                self.require_session(first(&args)?)?;
                ...
                self.active = None;
                self.db = None;
                self.set_last_catalog(None)?;
                Ok(Value::Null)
            }
            "darkroom:catalog-trash-asset" => {
                self.require_session(first(&args)?)?;
                Err(
                    "Removing files from disk is unavailable until recovery can be guaranteed."
                        .into(),
                )
            }
            "darkroom:catalog-remove" => self.remove(first(&args)?),
            "darkroom:catalog-query" => {
                self.require_session(first(&args)?)?;
                self.query(first(&args)?)
            }
            "darkroom:catalog-apply" => {
                let request = first(&args)?;
                self.require_session(request)?;
                let result = self.apply(request)?;
                if let Some(name) = request["mutations"]
                    .as_array()
                    .and_then(|items| {
                        items
                            .iter()
                            .rev()
                            .find(|item| item["kind"] == "rename-catalog")
                    })
                    .and_then(|item| item["displayName"].as_str())
                {
                    if let Some(entry) =
                        self.registry["catalogs"].as_array_mut().and_then(|items| {
                            items
                                .iter_mut()
                                .find(|item| item["catalogId"] == request["catalogId"])
                        })
                    {
            ...
            | "darkroom:catalog-fingerprint-cancel" => {
                self.fingerprint_dispatch(command, first(&args)?)
            }
            "darkroom:catalog-relink-files-prepare"
            | "darkroom:catalog-relink-files-apply"
            | "darkroom:catalog-relink-files-cancel" => self.relink_dispatch(command, &args),
            "darkroom:catalog-import-prepare"
            | "darkroom:catalog-import-review"
            | "darkroom:catalog-import-cancel" => self.import_dispatch(command, &args),
            ...
            | "darkroom:catalog-auto-import-open-ingress" => {
                self.auto_dispatch(command, first(&args)?)
            }
            "darkroom:catalog-admin-inspect"
            | "darkroom:catalog-admin-backup"
            | "darkroom:catalog-admin-export"
            | "darkroom:catalog-admin-validate-package"
            | "darkroom:catalog-admin-import-as-new"
            | "darkroom:catalog-admin-optimize-preview"
            | "darkroom:catalog-admin-optimize"
            | "darkroom:catalog-admin-get-backup-policy"
            | "darkroom:catalog-admin-set-backup-policy"
            | "darkroom:catalog-admin-run-scheduled-backup" => self.admin_dispatch(command, &args),
            ...
            | "darkroom:develop-history-projection-set" => self.history_dispatch(command, &args),
            _ => Err(format!("Unsupported catalog command: {command}")),
        }
    }

    pub fn active_database(&self) -> Option<&Connection> {
        self.db.as_ref()
    }

    pub fn set_emit(&mut self, callback: Arc<dyn Fn(&str, Value) + Send + Sync>) {
        self.emit = Some(callback);
    }

    pub fn active_path(&self) -> Option<&Path> {
        self.active.as_ref().map(|a| a.database_path.as_path())
    }

    fn db(&self) -> Result<&Connection, String> {
        self.db.as_ref().ok_or_else(|| "No catalog is open.".into())
    }

    fn registry_path(&self) -> PathBuf {
        self.user_data.join("catalog-registry.json")
    }
    ...
    fn bootstrap(&mut self) -> Result<Value, String> {
        let migration_recovery = if self.registry["catalogs"]
            .as_array()
        ...
        let settings = fs::read_to_string(self.user_data.join("settings.json"))
            .ok()
            .and_then(|raw| serde_json::from_str::<Value>(&raw).ok());
        let last = settings
            .as_ref()
            .and_then(|v| v.get("lastCatalogId"))
        ...
        Ok(
            json!({"catalogs":self.catalog_list(), "session":self.active_session()?, "recovery":recovery}),
        )
    }
    ...
    fn active_session(&self) -> Result<Value, String> {
        ...
        let roots = rows(
            self.db()?,
            "SELECT catalog_id AS catalogId, root_id AS rootId, label FROM roots WHERE catalog_id = ? ORDER BY root_id",
    ...
    fn activate(&mut self, catalog_id: &str, path: PathBuf) -> Result<Value, String> {
        ...
        self.relink_drafts.clear();
        if !path.is_file() {
            return Err("Catalog file is missing.".into());
        }
        ...
        let version: i64 = db
            ...
            .map_err(|e| e.to_string())?;
        if app_id != 1_146_243_891 || version != 3 {
            return Err("Catalog database format is invalid.".into());
        }
        let actual_id: String = db
            .query_row("SELECT catalog_id FROM catalog_meta", [], |row| row.get(0))
            .map_err(|e| e.to_string())?;
        if actual_id != catalog_id {
            return Err("Catalog database identity mismatch.".into());
        }
        upgrade_v3(&db)?;
        ...
        self.db = Some(db);
        self.active = Some(ActiveCatalog {
        ...
        let entry = self.registry["catalogs"]
            .as_array_mut()
            .ok_or("Catalog registry is invalid.")?
            .iter_mut()
            .find(|v| v["catalogId"] == catalog_id)
            .ok_or("Catalog is not registered.")?;
        entry["health"] = json!("healthy");
        entry["lastOpenedAt"] = json!(now());
    ...
    fn open(&mut self, catalog_id: &str) -> Result<Value, String> {
        let item = self.registry["catalogs"]
            .as_array()
            .ok_or("Catalog registry is invalid.")?
            .iter()
            .find(|v| v["catalogId"] == catalog_id)
            .ok_or("Catalog was not found.")?;
    ...
    fn create(&mut self, args: &Value) -> Result<Value, String> {
        ...
        if !canonical.is_dir() {
            return Err("Catalog root must be a folder.".into());
        }
        ...
        let database_path = self
            .user_data
            .join("catalogs")
            .join(format!("{catalog_id}.sqlite"));
        fs::create_dir_all(database_path.parent().ok_or("Catalog path is invalid.")?)
            .map_err(|e| e.to_string())?;
    ...
    pub fn require_session(&self, request: &Value) -> Result<(), String> {
        ...
        if string(request, "catalogId")? != active.catalog_id
            || string(request, "sessionId")? != active.session_id
        {
    ...
    pub fn resolve_asset(&self, request: &Value) -> Result<Value, String> {
        ...
        if row["rootHealth"] != "online" {
            return Err("Asset root is not online.".into());
        }
        ...
        Ok(
            json!({"catalogId":catalog_id,"assetId":asset_id,"rootId":row["rootId"],"canonicalRootPath":canonical_root,"relativePath":relative}),
        )
    }
    ...
    fn remove(&mut self, input: &Value) -> Result<Value, String> {
        ...
        if self
            .active
            .as_ref()
            .is_some_and(|active| active.catalog_id == id)
        {
            return Err("Close the active catalog before removing it.".into());
        }
        let catalogs = self.registry["catalogs"]
            .as_array_mut()
            .ok_or("Catalog registry is invalid.")?;
        let position = catalogs
            .iter()
            .position(|v| v["catalogId"] == id)
            .ok_or("Catalog was not found.")?;
        let entry = catalogs[position].clone();
        if input["deleteFile"] == true {
            if input["confirmation"] != entry["displayName"] {
                return Err("Catalog deletion confirmation does not match its name.".into());
            }
    ...
    fn relink_root(&mut self, args: &Value) -> Result<Value, String> {
        ...
        let entry = self.registry["catalogs"]
            .as_array()
            .and_then(|a| a.iter().find(|v| v["catalogId"] == catalog_id))
            .ok_or("Catalog is not registered.")?;
        let catalog = json!({"catalogId":entry["catalogId"],"displayName":entry["displayName"],"health":entry["health"],"lastOpenedAt":entry["lastOpenedAt"]});
        Ok(json!({"catalog":catalog,"session":session}))
    }
    ...
    fn revision(&self, catalog_id: &str) -> Result<i64, String> {
        self.db()?
            .query_row(
                "SELECT revision FROM catalog_meta WHERE catalog_id = ?",
                [catalog_id],
                |r| r.get(0),
            )
            .map_err(|e| e.to_string())
