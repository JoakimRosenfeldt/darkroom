...
pub(super) fn format_id(name: &str) -> Option<&'static str> {
    ...
    match extension.as_str() {
        ...
        "orf" => Some("orf"),
        "rw2" => Some("rw2"),
        "heif" | "heic" | "hif" => Some("heif"),
        "tif" | "tiff" => Some("tiff"),
        "psd" | "psb" => Some("psd"),
        "jxl" => Some("jxl"),
        "mov" | "mp4" | "m4v" | "avi" | "mkv" => Some("video"),
        _ => None,
    }
}
...
#[cfg(not(unix))]
fn local_file_id(_: &fs::Metadata) -> Value {
    Value::Null
}
...
fn update(
    ...
    if let Ok(mut all) = operations.lock() {
        if let Some(operation) = all.get_mut(operation_id) {
            operation.snapshot = snapshot.clone();
        }
    }
}
...
fn emit(
    ...
    if let Some(callback) = callback {
        ...
        callback(
            ...
            json!({"catalogId":snapshot["catalogId"],"sessionId":snapshot["sessionId"],"operationId":snapshot["operationId"],"sequence":number,"kind":kind,"payload":payload}),
        );
    }
}

pub(super) fn scan_folder<F: FnMut(&str, usize, usize, usize, &str)>(
    root: &Path,
    cancelled: &AtomicBool,
    timeout: Duration,
    mut progress: F,
) -> Result<(Vec<Value>, usize, usize, String), String> {
    let started = Instant::now();
    let canonical = fs::canonicalize(root).map_err(|e| e.to_string())?;
    if canonical != root {
        return Err("Scan root changed during scan.".into());
    }
    ...
    while let Some((directory, relative)) = queue.pop_front() {
        ...
        if !metadata.is_dir() || metadata.file_type().is_symlink() {
            return Err("Scan directory changed to a non-directory.".into());
        }
        directories += 1;
        if directories > 10_000 {
            return Err("Folder is too large or contains too many subfolders to scan.".into());
        }
        ...
        for item in entries {
            ...
            let Some(format_id) = format_id(&name) else {
                continue;
            };
            files += 1;
...
impl CatalogService {
    pub(super) fn cancel_all_scans(&mut self) {
        ...
        for handle in self.scan_handles.drain(..) {
            let _ = handle.join();
        }
    }
    ...
    pub(super) fn start_scan(&mut self, input: &Value) -> Result<Value, String> {
        ...
        if root["health"] != "online" {
            return Err("Scan root is offline.".into());
        }
        ...
        self.operations.lock().map_err(|e| e.to_string())?.insert(
            operation_id.clone(),
            ScanOperation {
        ...
        let worker = std::thread::spawn(move || {
            ...
            let result = scan_folder(
                ...
                &cancelled,
                timeout,
                |phase, directories, files, accepted, current| {
                    ...
                    emit(
                        ...
                        &progress_snapshot,
                        "scan-progress",
                        json!({"phase":phase,"directoriesVisited":directories,"filesConsidered":files,"acceptedCount":accepted,"currentPath":current}),
                    );
                },
            );
            ...
            let status = match &outcome {
                ...
                Err(error) if error == "timed-out" => "timed-out",
                Err(_) => "failed",
            };
            progress_snapshot["status"] = json!(status);
            if let Err(error) = &outcome {
                if status == "failed" {
                    progress_snapshot["errorMessage"] = json!(error);
                }
            }
            update(&operations, &operation_id, &progress_snapshot);
            ...
            if status == "failed" {
                payload["errorMessage"] = progress_snapshot["errorMessage"].clone();
            }
            emit(
                &callback,
                &sequence,
    ...
    pub(super) fn scan_operation(&self, input: &Value, wait: bool) -> Result<Value, String> {
        ...
        loop {
            let snapshot = {
                let all = self.operations.lock().map_err(|e| e.to_string())?;
                all.get(operation_id)
                    .ok_or("Scan operation is missing.")?
                    .snapshot
                    .clone()
            };
            if !wait || snapshot["status"] != "running" {
                return Ok(snapshot);
            }
