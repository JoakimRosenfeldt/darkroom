...
struct MetadataJob { catalog_id: String, session_id: String, cancelled: Arc<AtomicBool> }
static JOBS: OnceLock<Mutex<HashMap<String,MetadataJob>>> = OnceLock::new();
fn jobs() -> &'static Mutex<HashMap<String,MetadataJob>> { JOBS.get_or_init(||Mutex::new(HashMap::new())) }

struct MetadataTarget { entry_id: String, location: Option<Value>, size: f64, modified_at: f64 }
...
impl Drop for MetadataPlan {
    fn drop(&mut self) { if let Ok(mut jobs)=jobs().lock() { jobs.remove(&self.operation_id); } }
}
...
fn cache_path(plan:&MetadataPlan,entry:&str)->PathBuf {
    plan.cache_root.join(&plan.catalog_id).join(format!("{entry}.json"))
}
fn adapter(location:&Value)->&'static str {
    ...
    if matches!(extension.as_str(),"3fr"|"arw"|"cr2"|"cr3"|"dng"|"erf"|"fff"|"iiq"|"kdc"|"mef"|"mos"|"mrw"|"nef"|"nrw"|"orf"|"pef"|"raf"|"raw"|"rw2"|"rwl"|"sr2"|"srf"|"srw"|"x3f") {"1.0.0-raw"} else {"1.0.0-standard"}
}
fn js_number(value:f64)->String { ryu_js::Buffer::new().format(value).to_owned() }
fn signature(size:f64,modified:f64)->String { format!("{}:{}",js_number(size),js_number(modified)) }
...
fn read_cache(plan:&MetadataPlan,target:&MetadataTarget,digest:&str)->Option<Value> {
    ...
    if parsed["version"]!=1 || parsed["sourceSha256"]!=digest || parsed["parserVersion"]!=PARSER_VERSION || parsed["adapterVersion"]!=adapter(location) { return None; }
    let mut analysis=parsed.get("analysis")?.clone();
    if !analysis.is_object() { return None; }
...
fn write_cache(plan:&MetadataPlan,target:&MetadataTarget,analysis:&Value) {
    if !analysis["sourceSha256"].is_string() || !analysis["parserVersion"].is_string() { return; }
    ...
    if let Ok(bytes)=serde_json::to_vec(&payload) {
        if let Ok(mut file)=fs::OpenOptions::new().write(true).create_new(true).open(&temp) {
            if file.write_all(&bytes).is_ok() { let _=fs::rename(&temp,&destination); }
        }
    }
    let _=fs::remove_file(temp);
}
fn analyze(plan:&MetadataPlan,target:&MetadataTarget)->Value {
    let Some(location)=&target.location else {return failure(target,None,false)};
    let path=match crate::native::resolve_asset_path(location) {Ok(path)=>path,Err(_)=>return failure(target,None,false)};
    let digest=match crate::native::metadata_sha256(&path) {Ok(hash)=>hash,Err(_)=>return failure(target,None,false)};
    if !plan.force { if let Some(cached)=read_cache(plan,target,&digest) {return cached;} }
...
pub fn run_metadata_analysis(plan:MetadataPlan)->Result<Value,String> {
    ...
    if let Some(emit)=&plan.emit {emit("darkroom:catalog-metadata-analysis-progress",report.clone());}
    for target in &plan.targets {
        if plan.cancelled.load(std::sync::atomic::Ordering::SeqCst) {break;}
        let analysis=analyze(&plan,target);
        if analysis["error"].is_string() {failed+=1;}
...
impl CatalogService {
    pub fn prepare_metadata_analysis(&self,request:&Value)->Result<MetadataPlan,String> {
        ...
        let ids=request["entryIds"].as_array().filter(|items|items.len()<=50_000).ok_or("Metadata analysis entryIds are invalid.")?;
        let mut seen=HashSet::new();
        let mut targets=Vec::new();
        for value in ids {
            let entry_id=value.as_str().ok_or("Metadata analysis entryIds are invalid.")?;
            Uuid::parse_str(entry_id).map_err(|_|"Metadata analysis entryIds are invalid.")?;
            if !seen.insert(entry_id.to_owned()) {continue;}
            let row=one(self.db()?,"SELECT a.root_id AS rootId,a.relative_path AS relativePath,a.observed_byte_length AS size,a.observed_modified_at AS modifiedAt,a.health,a.camera_make AS cameraMake,a.camera_model AS cameraModel,a.lens_model AS lens,r.canonical_path AS canonicalRootPath,r.health AS rootHealth FROM assets a JOIN roots r ON r.catalog_id=a.catalog_id AND r.root_id=a.root_id WHERE a.catalog_id=? AND a.asset_id=?",vec![SqlValue::Text(catalog_id.clone()),SqlValue::Text(entry_id.into())])?;
            let size=row.as_ref().and_then(|row|row["size"].as_f64()).unwrap_or(0.0);
            let modified_at=row.as_ref().and_then(|row|row["modifiedAt"].as_f64()).unwrap_or(0.0);
            ...
            targets.push(MetadataTarget{entry_id:entry_id.into(),location,size,modified_at});
        }
        let mut registry=jobs().lock().map_err(|e|e.to_string())?;
        if registry.contains_key(&operation_id) {return Err("Metadata analysis operation is already active.".into())}
        if registry.values().any(|job|job.catalog_id==catalog_id) {return Err("Metadata analysis is already active for this catalog.".into())}
        let cancelled=Arc::new(AtomicBool::new(false));
        registry.insert(operation_id.clone(),MetadataJob{catalog_id:catalog_id.clone(),session_id:session_id.clone(),cancelled:cancelled.clone()});
        Ok(MetadataPlan{catalog_id,session_id,operation_id,targets,cache_root:self.user_data.join("metadata-cache"),cancelled,emit:self.emit.clone(),force:request["force"]==true})
    }
    pub fn cancel_metadata_analysis(&self,request:&Value)->Result<Value,String> {
        ...
        if let Some(job)=registry.get(&operation) {
            if job.catalog_id==string(request,"catalogId")? && job.session_id==string(request,"sessionId")? {
