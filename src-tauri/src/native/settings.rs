...
fn valid_size(value: &Value) -> Value {
    match value.get("mode").and_then(Value::as_str) {
        ...
        Some("long-edge" | "longEdge") => {
            let pixels = value
                .get("pixels")
                .or_else(|| value.get("longEdge"))
                .and_then(Value::as_u64)
                .unwrap_or(0);
            if !(1..=100_000).contains(&pixels) {
                return json!({"mode":"original"});
            }
            let mut size = json!({"mode":"long-edge","pixels":pixels});
            if let Some(flag) = value.get("neverUpscale").and_then(Value::as_bool) {
                size["neverUpscale"] = json!(flag);
            }
            size
        }
        Some("fit") => {
...
fn normalize_export(input: &Value) -> Value {
    let format = input
        ...
        .filter(|v| matches!(*v, "jpeg" | "png" | "webp" | "avif" | "tiff"))
        .unwrap_or("jpeg");
    let quality = input
        .get("quality")
        .and_then(Value::as_u64)
    ...
    let metadata = input
        .get("metadata")
        .and_then(Value::as_str)
        .filter(|v| matches!(*v, "none" | "copyright"))
        .unwrap_or("all");
    let suffix = input
        .get("suffix")
        .and_then(Value::as_str)
        .filter(|v| v.len() <= 200 && !v.contains(['\0', '/', '\\']) && !v.contains(".."))
        .unwrap_or("-darkroom");
    let conflict = input
        .get("conflict")
        .and_then(Value::as_str)
        .filter(|v| matches!(*v, "rename" | "skip" | "replace"))
        .unwrap_or("rename");
...
pub fn set_export_options(root: &Path, options: Value) -> Result<Value, String> {
    ...
    if let Some(input) = options.as_object() {
...
pub fn get_clipboard_groups(root: &Path) -> Result<Value, String> {
    ...
    Ok(validate_groups(groups)
        .unwrap_or_else(|_| json!(["basic", "mixer", "effects", "tone-curves"])))
}
...
fn validate_groups(value: &Value) -> Result<Value, String> {
    ...
    if values.is_empty() || values.len() > CLIPBOARD_GROUPS.len() {
    ...
    for item in values {
        let name = item
            .as_str()
            .ok_or("Develop clipboard groups are invalid.")?;
        if !CLIPBOARD_GROUPS.contains(&name) || !seen.insert(name) {
            return Err("Develop clipboard groups are invalid.".into());
        }
    }
    Ok(value.clone())
