...
impl Fact {
    fn json(&self) -> Value {
        json!({"kind":"value","value":self.value,"source":self.source,"tag":self.tag})
    }
}
fn absent() -> Value {
    json!({"kind":"absent"})
}
fn field(fact: Option<&Fact>) -> Value {
    fact.map(Fact::json).unwrap_or_else(absent)
}
fn first(facts: &[Option<Fact>]) -> Option<Fact> {
    facts.iter().flatten().next().cloned()
}
...
fn gps_coordinate(data: &exif::Exif, tag: Tag, reference: Tag, name: &'static str) -> Option<Fact> {
    ...
    if (tag == Tag::GPSLatitude && signed.abs() > 90.0)
        || (tag == Tag::GPSLongitude && signed.abs() > 180.0)
    {
        return None;
    }
...
fn gps_altitude(data: &exif::Exif) -> Option<Fact> {
    ...
    let below = data
        ...
        == 1;
    if below {
        altitude.value = json!(-altitude.value.as_f64()?);
    }
    Some(altitude)
}
fn xmp_value(value: XmpValue) -> Value {
    match value {
        ...
        XmpValue::Structure(values) => Value::Object(
            values
                .into_iter()
                .map(|(key, value)| (key, xmp_value(value)))
                .collect(),
        ),
...
fn normalized_text(value: Value) -> Option<String> {
    match value {
        ...
        Value::Object(mut value) => value
            .remove("value")
            .or_else(|| value.remove("x-default"))
...
fn xmp_list(
    ...
    let list = match value {
        XmpValue::Array(values) => values
            .into_iter()
            .filter_map(|value| normalized_text(xmp_value(value)))
            .collect::<Vec<_>>(),
...
fn iptc_list(data: Option<&iptc::IPTC>, tag: iptc::IPTCTag, name: &'static str) -> Option<Fact> {
    let values = data?
        ...
        .get(&tag)?
        .iter()
        .filter_map(|value| normalized_text(json!(value)))
        .collect::<Vec<_>>();
    if values.is_empty() {
        None
    } else {
...
fn claim(field_name: &str, value: &Fact) -> Value {
    json!({"field":field_name,"value":value.value,"source":value.source,"tag":value.tag})
}
fn claims(field_name: &str, values: &[Option<Fact>], target: &mut Vec<Value>) {
    for value in values.iter().flatten() {
        target.push(claim(field_name, value));
    }
}
...
fn capture_time(fact: Option<&Fact>, offset_fact: Option<&Fact>) -> (Value, Value, Value, Value) {
    let Some(fact) = fact else {
        return (absent(), Value::Null, Value::Null, Value::Null);
    };
    let Some(raw) = fact.value.as_str() else {
        return (absent(), Value::Null, Value::Null, Value::Null);
    };
    let malformed = || {
        (
            json!({"kind":"malformed","message":format!("{} is not a valid capture time.",fact.tag)}),
            Value::Null,
            Value::Null,
            Value::Null,
        )
    ...
    let Some((year, month, day, hour, minute, second)) = parse(0, 4)
        ...
        .zip(parse(14, 16))
        .zip(parse(17, 19))
        .map(|(((((y, m), d), h), minute), s)| (y, m, d, h, minute, s))
    else {
        return malformed();
    };
    if year < 1
    ...
    let milliseconds = if bytes.get(19) == Some(&b'.') {
        let digits = bytes[20..]
            .iter()
    ...
    let offset = offset_fact
        ...
        .map(str::to_owned);
    let mut sort = (days_from_civil(year, month, day) * 86400 + hour * 3600 + minute * 60 + second)
        * 1000
        + milliseconds;
    ...
    (
        json!({"kind":"value","value":time,"source":fact.source,"tag":fact.tag}),
        json!(sort),
        json!(format!("{wall}{}", offset.as_deref().unwrap_or(""))),
        json!(if fact.tag == "DateTimeOriginal" {
            "date-time-original"
        } else {
            "create-date"
        }),
...
pub(crate) fn analyze_file_with_digest(
    location: &Value,
    ...
    let xmp = if xmp_file.open(&path).is_ok() {
        xmp_file.get_xmp()
    } else {
        None
    };
    ...
    let title_candidates = [
        xmp_fact(xmp, DC, "title"),
        iptc_fact(iptc, iptc::IPTCTag::ObjectName, "ObjectName")
            .or_else(|| iptc_fact(iptc, iptc::IPTCTag::Headline, "Headline")),
        exif.and_then(|exif| exif_text(exif, Tag::ImageDescription, "ImageDescription")),
    ];
    let caption_candidates = [
        xmp_fact(xmp, DC, "description"),
        iptc_fact(iptc, iptc::IPTCTag::Caption, "Caption")
            .or_else(|| iptc_fact(iptc, iptc::IPTCTag::LocalCaption, "LocalCaption")),
        exif.and_then(|exif| {
            exif_text(exif, Tag::ImageDescription, "ImageDescription")
                .or_else(|| exif_text(exif, Tag::UserComment, "UserComment"))
        }),
    ];
    let copyright_candidates = [
        xmp_fact(xmp, DC, "rights"),
        iptc_fact(iptc, iptc::IPTCTag::CopyrightNotice, "CopyrightNotice"),
        exif.and_then(|exif| {
            exif_text(exif, Tag::Copyright, "Copyright")
                .or_else(|| exif_text(exif, Tag::Artist, "Artist"))
        }),
    ];
    let keyword_candidates = [
        xmp_list(xmp, DC, "subject").or_else(|| xmp_list(xmp, LR, "hierarchicalSubject")),
        iptc_list(iptc, iptc::IPTCTag::Keywords, "Keywords")
            .or_else(|| iptc_list(iptc, iptc::IPTCTag::SubjectReference, "SubjectReference")),
    ];
    ...
    let iso =
        exif.and_then(|exif| exif_number(exif, Tag::PhotographicSensitivity, "ISOSpeedRatings"));
    let date = exif
        .and_then(|exif| exif_text(exif, Tag::DateTimeOriginal, "DateTimeOriginal"))
        .or_else(|| exif.and_then(|exif| exif_text(exif, Tag::DateTimeDigitized, "CreateDate")));
    let date_offset =
        exif.and_then(|exif| exif_text(exif, Tag::OffsetTimeOriginal, "OffsetTimeOriginal"));
    let (capture, capture_key, capture_display, capture_provenance) =
        capture_time(date.as_ref(), date_offset.as_ref());
    let fallback = &location["fallback"];
    let fallback_fact = |key: &str, tag: &'static str| -> Option<Fact> {
        fallback
            .get(key)
            .and_then(Value::as_str)
            .and_then(|value| text(value.to_owned(), "catalog-fallback", tag))
    };
    ...
    let width = exif
        .and_then(|exif| exif_number(exif, Tag::PixelXDimension, "exif.ExifImageWidth"))
        .map(|mut value| {
            value.source = "container";
            value
        });
    let height = exif
        .and_then(|exif| exif_number(exif, Tag::PixelYDimension, "exif.ExifImageHeight"))
        .map(|mut value| {
            value.source = "container";
            value
        });
    ...
    let city =
        xmp_fact(xmp, PHOTOSHOP, "City").or_else(|| iptc_fact(iptc, iptc::IPTCTag::City, "City"));
    let state = xmp_fact(xmp, PHOTOSHOP, "State")
        .or_else(|| iptc_fact(iptc, iptc::IPTCTag::ProvinceOrState, "ProvinceState"));
    let country = xmp_fact(xmp, PHOTOSHOP, "Country").or_else(|| {
        iptc_fact(
            iptc,
            iptc::IPTCTag::CountryOrPrimaryLocationName,
            "CountryName",
        )
    });
    let latitude = exif.and_then(|exif| {
        gps_coordinate(exif, Tag::GPSLatitude, Tag::GPSLatitudeRef, "GPSLatitude")
    });
    let longitude = exif.and_then(|exif| {
        gps_coordinate(
            exif,
            Tag::GPSLongitude,
            Tag::GPSLongitudeRef,
    ...
    Ok(
        json!({"cacheSignature":format!("{size}:{modified}"),"size":size,"modifiedAt":modified,"sourceSha256":digest,"parserVersion":source["parserVersion"],"adapterVersion":adapter,"cacheHit":false,"source":source,
