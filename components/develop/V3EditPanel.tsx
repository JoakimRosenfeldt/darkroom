  ...
  return (
    <>
      ...
      <div className="flex min-h-[58px] items-center gap-2 border-b border-lr-border-subtle px-4 py-3">
        <div className="min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            <span className="rounded bg-lr-selection px-1.5 py-0.5 font-mono text-[9px] text-lr-accent">
              v3
            </span>
          </div>
      ...
      <div
        ...
        role="tablist"
        aria-label="Develop v3 sections"
      >
        {TABS.map((tab) => (
          <button
            key={tab.id}
  ...
  const updateWhiteBalance = (field: "temperature" | "tint", value: number) => {
    ...
    replaceColor({
      ...
      whiteBalance: {
        ...
        mode: "custom",
        adjustment,
        resolved: resolveManualWhiteBalance(adjustment),
      },
...
function GeometryTab({ document }: { document: DevelopDocumentV3 }) {
  ...
  return (
    <PanelSection title="Geometry" onReset={() => reset("geometry")}>
      ...
      <SliderRow label="Horizontal" value={geometry.manualPerspective.horizontal} min={-100} max={100} onChange={(horizontal) => replace({ ...geometry, manualPerspective: { ...geometry.manualPerspective, horizontal, matrix: manualPerspectiveHomography(horizontal, geometry.manualPerspective.vertical) } }, "Adjust horizontal perspective")} />
      <SliderRow label="Vertical" value={geometry.manualPerspective.vertical} min={-100} max={100} onChange={(vertical) => replace({ ...geometry, manualPerspective: { ...geometry.manualPerspective, vertical, matrix: manualPerspectiveHomography(geometry.manualPerspective.horizontal, vertical) } }, "Adjust vertical perspective")} />
